...
allowable_dihedral_angles_graz = [
    ...
    (["C31","C30","N3","C29"],[],False),
    #1 - sulfone side chain - carbon part
    (["C29", "N3","C30","C35"], [],False),
    #2 - sulfone side chain - sulfur part
    (["S1","N4","C35","O6"], [],False),
    #3 - methoxy group distal
    (["C24","O9","C22","C23"],[],False),
    #4 - amide link in side chain
...
def main(dihedral_angle_atom_names, split_idx_atom_names, count, perturb_angle, input_struct_path, should_proximity_bond=False, should_sanitize=True):
    ...
    for i in range(len(dihedral_angle_atom_indices)):
        ...
        try:
            ...
            Chem.MolToPDBFile(base_mol, f"{output_conf_dir}graz_torsion_angle_perturb_{count}.pdb", confId=comp_id_pos)
            return base_mol
            ...
            # return edit_mol
                ...
                for bond in edit_mol.GetBonds():
                    ...
                    if dist > 3 and should_valid_struct:
                        print(f'suspicious bond distance {dist} angstroms for: graz_torsion_angle_perturb_{count}.pdb')
            ...
            Chem.MolToPDBFile(edit_mol, f"{output_conf_dir}graz_torsion_angle_perturb_{count}.pdb", confId=comp_id_pos)
            return edit_mol
...
for i in range(len(perturb_path)):
    ...
    dihedral_angle = allowable_dihedral_angles_graz[perturb_path[i]][0]
    split_atom_names = allowable_dihedral_angles_graz[perturb_path[i]][1]
    should_use_custom_dihed = allowable_dihedral_angles_graz[perturb_path[i]][2]
    input_struct_path =  "/Users/adam/Downloads/outputs_from_molec_replac/GRAZ_CONFORGE_TRIAL_1_20A/grazoprevir_conforge_125/grazoprevir_conforge_125_out.1.pdb"
    input_mtz = "/Users/adam/Downloads/inputs_for_molec_replac/grazoprevir_20A.mtz"
    count = 0   
    output_conf_dir = f"/Users/adam/Downloads/outputs_from_molec_replac/GRAZ_CUSTOM_CONF_TRIAL_1_20A/ROUND_{i}/"
    ...
    if i > 0:
        ...
        for k in range(len(sorted_output_conf)):
            ...
            if 'graz' not in phaser_input: continue
