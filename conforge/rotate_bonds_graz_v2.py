...
def main(dihedral_angle_atom_names, split_idx_atom_names, count, perturb_angle, input_struct_path, should_proximity_bond=False, should_sanitize=True):
    ...
    for i in range(len(dihedral_angle_atom_indices)):
        ...
        try:
            ...
            #             print(f'suspicious bond distance {dist} angstroms for: grazoprevir_torsion_angle_perturb_{count[0]}.pdb')
            #             print('skipping this one')
            #             return
            Chem.MolToPDBFile(base_mol, f"{output_conf_dir}grazoprevir_torsion_angle_perturb_{count[0]}.pdb", confId=comp_id_pos)
        
            return calc_new_fit(f"{output_conf_dir}grazoprevir_torsion_angle_perturb_{count[0]}.pdb",phaser_mtz,count, input_struct_path)
            ...
            Chem.MolToPDBFile(edit_mol, f"{output_conf_dir}grazoprevir_torsion_angle_perturb_{count[0]}.pdb", confId=comp_id_pos)
...
input_struct_path =  "/Users/adam/Downloads/outputs_from_molec_replac/GRAZ_CONFORGE_TRIAL_1_20A/grazoprevir_conforge_6/grazoprevir_conforge_6_out.1.pdb"
input_mtz = "/Users/adam/Downloads/inputs_for_molec_replac/grazoprevir_20A.mtz"
monomer_library = "/Users/adam/Downloads/outputs_from_molec_replac/phenix_refine_graz/eLBOW_44/elbow.UNK.001.cif"


perturb_path = rank_dihedrals(input_pdb_path_this=input_struct_path)
perturb_path = list(filter(lambda x: x[-2] > -5, perturb_path))
...
for i in range(len(perturb_path)):
    ...
    if perturb_path[i] != "REFINE" and perturb_path[i][-2] < -5: 
        continue
    output_conf_dir = f"/Users/adam/Downloads/outputs_from_molec_replac/GRAZ_CUSTOM_CONF_TRIAL_2_20A/ROUND_{i}/"
    count = [0,[]]
    ...
    if perturb_path[i] == "REFINE":
        ...
        for k in range(len(input_struct_path_list)):
            ...
            param_str = param_str.replace("REPLACE_UNIT_CELL", "6.850 17.450 34.510 90.00 90.00 90.00")
            param_str = param_str.replace("REPLACE_RFLAG_FRACTION", "0.025")
    ...
    # if i == 19:
        # input_struct_path_list = ["/Users/adam/Downloads/outputs_from_molec_replac/PAC_CUSTOM_CONF_TRIAL_1_20A/ROUND_18/grazoprevir_torsion_angle_perturb_6_out.1_refine_001.pdb"]
        # should_use_electron_difference = False
    if i > 0:
