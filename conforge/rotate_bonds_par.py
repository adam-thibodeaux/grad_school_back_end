...
allowable_dihedral_angles_paritaprevir = [
    #0 - planarization of pyridine ring
    (["C1","C2","O7","C28"],[], False),
    #1 - pyridine ring as a whole - fishy
    (["N1","C1","C2","O7" ],[["C3","C4"]], False),
    #2 - pyrimdine side chain as a whole
    (["C15","C16", "N4", "C18"],[["C15", "C14"]], False),
    #3 - pyrimidine side chain amide planarization
    (["O6","C18", "C19", "C20"],[], False),
    #4 - sulfone side chain as a whole
    (["N2","C6","C24","N3"],[["N2","C5"]], False),
    #5 - cyclopropane ring on sulfone side chain
    (["C24","N3","S1","C25"],[], False),
    #6 - free amide link in macrocycle - C-N
    (["C8","C6","N2","C5"],[["H41"]], True),
    #7 - end distal side of macrocycle core
    (["C9", "C10", "C11", "C12"],[["H11", "H46"]], True),
    #8 - end distal side of macrocycle core
    (["C10", "C11", "C12", "C13"],[["H12", "H47"]], True),
    #9 - end distal side of macrocycle core
    (["C11", "C12", "C13", "C14"],[["H13", "H48"]], True),
    #10 - end distal side of macrocycle core
    (["C12", "C13", "C14", "C15"],[["H14", "H49"]], True),
    #11 - end distal side of macrocycle core
    (["C13", "C14", "C15", "C16"],[["H15", "H50"]], True),
    #12 - proline ring in macrocycle
    (["O1","C17","N1","C1"],[["O7","C2"],["C4","C5"]], False),
    #13 - free amide link in macrocycle - C=O
    (["C6","N2","C5","C4"],[["O2"]], True),
    ]
# same dihedrals, just renamed from new order...
allowable_dihedral_angles_paritaprevir = [
    #0 - planarization of pyridine ring (fixed)
    (["C13","O5","C22","N5"],[], False),
    #1 - pyridine ring half axis (fixed)
    (["C22","O5","C13","C11" ],[], False),
    #2 - pyrimdine side chain half axis (fixed)
    (["C24","N4","C18","C16"],[], False),
    #3 - pyrimidine side chain perpendicular axis - flips amide (fixed)
    (["C18","N4","C24","O7"],[],False),
    #4 - pyrimidine side chain amide planarization (fixed)
    (["N4","C24","C29","N6"],[], False),
    #5 - sulfone side chain as a whole - no flip amide (fixed)
    (["O2","S1","N2","C4"],[], False),
    #6 - sulfone side chain as a whole - yes flip amide (fixed)
    (["S1","N2","C4","O1"],[],False),
    #7 - cyclopropane ring on sulfone side chain (fixed)
    (["O2","S1","C6","C7"],[], False),
    #8 - free amide link in macrocycle - C-N
    (["C8","C6","N2","C5"],[["H41"]], True),
    #9 - end distal side of macrocycle core
    (["C9", "C10", "C11", "C12"],[["H11", "H46"]], True),
    #10 - end distal side of macrocycle core
    ...
    #11 - end distal side of macrocycle core
    (["C11", "C12", "C13", "C14"],[["H13", "H48"]], True),
    #12 - end distal side of macrocycle core
    (["C12", "C13", "C14", "C15"],[["H14", "H49"]], True),
    #13 - end distal side of macrocycle core
    (["C13", "C14", "C15", "C16"],[["H15", "H50"]], True),
    #14 - proline ring in macrocycle
    (["O1","C17","N1","C1"],[["O7","C2"],["C4","C5"]], False),
    #15 - free amide link in macrocycle - C=O
    (["C6","N2","C5","C4"],[["O2"]], True),
    ]
all_conformers = []
...
def main(dihedral_angle_atom_names, split_idx_atom_names, count, perturb_angle, input_struct_path, should_proximity_bond=False, should_sanitize=True):
    ...
    for i in range(len(dihedral_angle_atom_indices)):
        orig_bond_num = len(base_mol.GetBonds())
        ...
        try:
            ...
            new_bond_num = len(base_mol.GetBonds())
            if should_valid_struct:
                if new_bond_num != orig_bond_num:
                    print("invalid bonding pattern found in new conformer")
            ...
            # return edit_mol
                ...
                new_bond_num = len(edit_mol.GetBonds())
                if should_valid_struct:
                    if new_bond_num != orig_bond_num:
                        print("invalid bonding pattern found in new conformer")
                        print('skipping this one')
...
perturb_path = [0,1,2,4,5,6,7]
for i in range(len(perturb_path)):
    should_skip_conf_gen = False
    dihedral_angle = allowable_dihedral_angles_paritaprevir[perturb_path[i]][0]
    split_atom_names = allowable_dihedral_angles_paritaprevir[perturb_path[i]][1]
    should_use_custom_dihed = allowable_dihedral_angles_paritaprevir[perturb_path[i]][2]
    input_struct_path =  "/Users/adam/Downloads/inputs_for_molec_replac/PAR_CONFORGE_TRIAL_1/paritaprevir_alpha_conforge_78.pdb"
    input_mtz = "/Users/adam/Downloads/inputs_for_molec_replac/paritaprevir_alpha_10A.mtz"
    count = 0   
    output_conf_dir = f"/Users/adam/Downloads/outputs_from_molec_replac/PAR_CUSTOM_CONF_TRIAL_1_10A/ROUND_{i}/"
    ...
    if i > 0:
        for input_struct_path_individ in input_struct_path_list:
            ...
            if i <= 1 and not should_use_custom_dihed:
                count = generate_conformers(count=count, step=30, dihedral_angle_atom_names=dihedral_angle,split_idx_atom_names=split_atom_names, input_struct_path=input_struct_path, should_proximity_bond=True)
            elif should_use_custom_dihed:
                count = show_dihedral_change.main(input_struct = input_struct_path_individ, atom_names = dihedral_angle,adjacent_atom_names=split_atom_names[0],output_path=output_conf_dir, count = count)
            else:
                count = generate_conformers(count=count, step=30, dihedral_angle_atom_names=dihedral_angle,split_idx_atom_names=split_atom_names, input_struct_path=input_struct_path, should_proximity_bond=False)
        timers = []
        sorted_output_conf = []
