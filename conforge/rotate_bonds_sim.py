...
def main(dihedral_angle_atom_names, split_idx_atom_names, count, perturb_angle, input_struct_path, should_proximity_bond=False, should_sanitize=True):
    ...
    for i in range(len(dihedral_angle_atom_indices)):
        ...
        try:
            ...
            Chem.MolToPDBFile(base_mol, f"{output_conf_dir}simeprevir_torsion_angle_perturb_{count}.pdb", confId=comp_id_pos)
            return base_mol
            ...
            # return edit_mol
                ...
                for bond in edit_mol.GetBonds():
                    ...
                    if dist > 3 and should_valid_struct:
                        print(f'suspicious bond distance {dist} angstroms for: simeprevir_torsion_angle_perturb_{count}.pdb')
            ...
            Chem.MolToPDBFile(edit_mol, f"{output_conf_dir}simeprevir_torsion_angle_perturb_{count}.pdb", confId=comp_id_pos)
            return edit_mol
...
for i in range(len(perturb_path)):
    ...
    input_struct_path =  "/Users/adam/Downloads/outputs_from_molec_replac/SIM_CONFORGE_TRIAL_2/simeprevir_conforge_709/simeprevir_conforge_709_out.1.pdb"
    input_mtz = "/Users/adam/Downloads/inputs_for_molec_replac/simeprevir.mtz"
    count = 0   
    output_conf_dir = f"/Users/adam/Downloads/outputs_from_molec_replac/SIM_CUSTOM_CONF_TRIAL_2/ROUND_{i}/"
    ...
    if i > 0:
        for input_struct_path_individ in input_struct_path_list:
            ...
            if i <= 1 and not should_use_custom_dihed:
                ...
                count = generate_conformers(count=count, step=10, dihedral_angle_atom_names=dihedral_angle,split_idx_atom_names=split_atom_names, input_struct_path=input_struct_path, should_proximity_bond=False)
        timers = []
        sorted_output_conf = []
        ...
        for k in range(len(sorted_output_conf)):
            ...
            if 'sim' not in phaser_input: continue
