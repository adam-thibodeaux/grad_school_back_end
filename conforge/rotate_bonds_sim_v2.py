...
allowable_dihedral_angles_sim = [
    #0 - pyridine side chain rotation above/below macrocycle
    (["C18","O6","C27","C28"],[],False),
    # 1 - methoxy linker on pyridine side chain
    (["C28","C27","O6","C18"],[[]],True),
    #2 - sulfone side chain as a whole
    (["C11","C12","C13","O2"],[["N3","S2"]],False),
    #3 - sulfone side chain cyclopropyl group
    (["S2","C16","C37","C17"],[["C16","C17"]],False),
    #4 - macrocycle amide - C=0
    (["C7","C20","C19","N5"],[["O5"]],True),
    #5 - macrocycle amide - N-H
    (["C10","C12","N5","C19"],[["H32"]],True),
    #6 - amide link on sulfone side chain
    (["S2","N3","C13","C12"],[],False),
    #7 - propyl group on pyridine side chain
    (["C22","C15","C33","N2"],[],False),
    #8 - pyridine side chain planarization
    (["C34","C18","O6","C27"],[],False),
    #9 - amide link in macrocycle (rdkit)
    (["C10","C12","N5","C19"],[["C19","C20"]],False),
    #10 - macrycle distal core
    (["C4","C6","C8","C9"],[["H15"]],True),
    #11 -
    "REFINE",
    #12 - macrocycle distal/medial core
    (["C8","C6","C4","C3"],[["H8","H9"]],True),
    #13 - distal alkene macrocycle
    (["C6","C4","C3","C2"],[["H6","H7"]],True),
    #14 - macrocycle medial core
    (["C9","C8","C6","C4"],[["H12","H13"]], True),
    #15 - medial alkene macrocycle
    (["C4","C3","C2","N4"],[["H4","H5"]],True),
    #16 - sulfone specific rotation
    (["C13","N3","S2","C16"],[],False),
    #17 - C=0 next to substituted amine in macrocycle
    (["C2","N4","C25","C7"],[["O3"]],True),
    #18 - N-C next to substituted amine in macrocycle
    (["C3","C2","N4","C25"],[["C1","H1","H2","H3"]],True),
    #19 - super medial macrocycle core
    (["C4","C3","C2","N4"],[["H4","H5"]],True),
    #20 - planarization of cyclopentyl ring
    (["O3","C25","C7","C28"],[["C19","C20"]],False)
    ]
all_conformers = []
...
def main(dihedral_angle_atom_names, split_idx_atom_names, count, perturb_angle, input_struct_path, should_proximity_bond=False, should_sanitize=True):
    ...
    for i in range(len(dihedral_angle_atom_indices)):
        ...
        try:
            ...
            Chem.MolToPDBFile(base_mol, f"{output_conf_dir}simeprevir_torsion_angle_perturb_{count[0]}.pdb", confId=comp_id_pos)
        
            return calc_new_fit(f"{output_conf_dir}simeprevir_torsion_angle_perturb_{count[0]}.pdb",phaser_mtz,count, input_struct_path)
            ...
            # return edit_mol
                ...
                for bond in edit_mol.GetBonds():
                    ...
                    if dist > 3 and should_valid_struct:
                        print(f'suspicious bond distance {dist} angstroms for: simeprevir_torsion_angle_perturb_{count[0]}.pdb')
            ...
            Chem.MolToPDBFile(edit_mol, f"{output_conf_dir}simeprevir_torsion_angle_perturb_{count[0]}.pdb", confId=comp_id_pos)
...
for i in range(len(perturb_path)):
    ...
    dihedral_angle = allowable_dihedral_angles_sim[perturb_path[i]][0]
    split_atom_names = allowable_dihedral_angles_sim[perturb_path[i]][1]
    should_use_custom_dihed = allowable_dihedral_angles_sim[perturb_path[i]][2]
    
    input_struct_path =  "/Users/adam/Downloads/outputs_from_molec_replac/SIM_CONFORGE_TRIAL_2/simeprevir_conforge_328/simeprevir_conforge_328_out.1.pdb"
    input_mtz = "/Users/adam/Downloads/inputs_for_molec_replac/simeprevir.mtz"
    monomer_library = "/Users/adam/Downloads/outputs_from_molec_replac/phenix_refine_sim/eLBOW_2/elbow.UNK.simeprevir_pdb.001.cif"
    count = [0,[]]
    output_conf_dir = f"/Users/adam/Downloads/outputs_from_molec_replac/SIM_CUSTOM_CONF_TRIAL_3/ROUND_{i}/"
    ...
    if allowable_dihedral_angles_sim[perturb_path[i]] == "REFINE":
    ...
    if i > 0:
        for input_struct_path_individ in input_struct_path_list:
            ...
            if i > 1 and allowable_dihedral_angles_sim[perturb_path[i-1]] == "REFINE":
            ...
            elif i <= 1 and not should_use_custom_dihed:
                count = generate_conformers(count=count, step=20, dihedral_angle_atom_names=dihedral_angle,split_idx_atom_names=split_atom_names, input_struct_path=input_struct_path, should_proximity_bond=True)
